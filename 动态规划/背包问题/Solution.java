...
class Solution3{
    ...
    public static int dp(int[] weight,int[] value,int numberOfThings, int capacity){
        ...
        for(int i=0;i<numberOfThings;i++){
            ...
            // 详细解释：在更新 dp[j] 时，我们需要确保使用的是不包含当前物品的旧值。倒序遍历可以保证在计算 dp[j] 时，dp[j-w[i]] 仍然是不包含当前物品 i 的状态。
            // 如果是正序遍历，一旦 dp[j] 被更新，较大的 j 值在后续迭代中可能会错误地重复使用同一物品，因为它们会使用已经考虑了当前物品的 dp[j-w[i]] 值。
